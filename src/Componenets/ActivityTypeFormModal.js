...
const ActivityTypeFormModal = (props) => {
    ...
    const submit = (event) => {
        ...
        if (errors == null) {
            ...
            if (modelFields._id === undefined) {
                request = activityTypesActions.create(modelFields);
            } else {
                request = activityTypesActions.update(modelFields._id, modelFields);
            }
            request.then((res) => {
                toast('Activity Type has been saved');
                hideLoading();
                clearModelFields();
                closeThisModal(true);
            }).catch((error) => {
    ...
    const closeThisModal = (thingsChanged = false) => {
        closeModal("activityTypeFormModal");
    ...
    return (
        <>
            <div className="modal fade" id="activityTypeFormModal" tabIndex="-1"
                 aria-labelledby="activityTypeFormModalLabel"
                 aria-hidden="true">
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title"
                                id="activityTypeFormModalLabel">{modelFields._id ? 'Update Activity Type' : 'Add New Activity Type'}</h5>
