...
import {formatDate, handleRequestError, hideLoading, openModal, showLoading} from "../Utils/Helpers";
import {toast} from "react-toastify";
import ActivityTypeFormModal from "../Componenets/ActivityTypeFormModal";

export const ActivityTypes = (props) => {
    ...
    const handleEdit = (model) => {
        showLoading();
        activityTypesActions.view(model._id).then(res => {
            hideLoading();
            setModel(res);
            openFormModal();
        }).catch(err => {
    ...
    const handleDelete = (model) => {
        if (window.confirm("Are you sure you want to delete?") === true) {
            showLoading();
            activityTypesActions.deleteRecord(model._id).then(res => {
                toast('Activity Type has been deleted');
                getModels(1);
            }).catch(err => {
    ...
    return (
        <>
            <ActivityTypeFormModal formModel={model} onClose={formModelClosed}/>
